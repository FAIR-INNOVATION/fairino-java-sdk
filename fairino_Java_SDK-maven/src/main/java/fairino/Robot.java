...
>>>>>>> 3.9.9
    ...
    public int MoveL(JointPos joint_pos, DescPose desc_pos, int tool, int user, double vel, double acc, double ovl, double blendR, int blendMode,ExaxisPos epos, int search, int offset_flag, DescPose offset_pos, int overSpeedStrategy, int speedPercent)
    {
        int errcode = MoveL(joint_pos, desc_pos, tool, user, vel, acc, ovl, blendR, blendMode, epos, search, offset_flag, offset_pos, 100,0, overSpeedStrategy, speedPercent);
    ...
    public int MoveL(DescPose desc_pos, int tool, int user, double vel, double acc, double ovl, double blendR, int blendMode,ExaxisPos epos, int search, int offset_flag, DescPose offset_pos, int config, int overSpeedStrategy, int speedPercent) {
        ...
        errcode = MoveL(jPos, desc_pos, tool, user, vel, acc, ovl, blendR, blendMode, epos, search, offset_flag, offset_pos, config, 0, overSpeedStrategy, speedPercent);
        return errcode;
    }
...
//    }
    ...
    public int ServoMoveStart(int comType)
    {
        ...
        try
        {
            if (comType == 0) {
                ...
                Frame frame = new Frame(cmdFrameCnt, 689, cmdStr);
                int rtn = udpCmdClient.sendFrame(frameHandle.packFrame(frame));
                if (rtn != 0) {
    ...
    public int ServoMoveEnd(int comType)
    {
        ...
        {   if (comType == 0) {
                ...
                Frame frame = new Frame(cmdFrameCnt, 690, cmdStr);
                int rtn = udpCmdClient.sendFrame(FrameHandle.packFrame(frame));
                if (rtn != 0) {
                    return RobotError.ERR_SOCKET_SEND_FAILED;
                }
     ...
     * @return  错误码
     */
    public int ServoJ(JointPos joint_pos, ExaxisPos axisPos, double acc, double vel, double cmdT, double filterT, double gain, int id,int comType)
    {
        if (IsSockComError())
        {
            return sockErr;
...
>>>>>>> 3.9.9
                ...
                String jointStr = String.format("{%.3f,%.3f,%.3f,%.3f,%.3f,%.3f}",
                        joint_pos.J1, joint_pos.J2, joint_pos.J3, joint_pos.J4, joint_pos.J5, joint_pos.J6);
                
                String axisStr = String.format("{%.3f,%.3f,%.3f,%.3f}",
                        axisPos.axis1, axisPos.axis2, axisPos.axis3, axisPos.axis4);

                String cmdStr = String.format("ServoJ(%s,%s,%f,%f,%f,%f,%f,%d)",
                        jointStr, axisStr, acc, vel, cmdT, filterT, gain, id);
                
                Frame frame = new Frame(cmdFrameCnt, 476, cmdStr);
                int rtn = udpCmdClient.sendFrame(FrameHandle.packFrame(frame));
                if (rtn != 0) {
                    return RobotError.ERR_SOCKET_SEND_FAILED;
                }
...
=======
        ...
        {
            if (comType == 0) {
                List<Object[]> jointPosList = new ArrayList<>();
                for (JointPos jp : joint_pos) {
                    jointPosList.add(new Object[]{jp.J1, jp.J2, jp.J3, jp.J4, jp.J5, jp.J6});
                }
                Object[] axis = {axisPos.axis1, axisPos.axis2, axisPos.axis3, axisPos.axis4};
                Object[] params = new Object[] {jointPosList, axis, acc, vel, cmdT, filterT, gain, id};
                Object result = client.execute("ServoJ" , params);
                int rtn;
                if (result instanceof Integer) {
                    rtn = (Integer) result;
                    ...
                    Object rtnObj = resultMap.get("rtn");
                    Object countObj = resultMap.get("servoJCmdCount");
                    if (rtnObj instanceof Integer) {
                        rtn = (Integer) rtnObj;
                    } else {
                        rtn = RobotError.ERR_RPC_ERROR;
                    }
                    if (countObj instanceof Integer && servoJCmdCount != null && servoJCmdCount.length > 0) {
                        servoJCmdCount[0] = (Integer) countObj;
                    }
                } else {
                    rtn = RobotError.ERR_RPC_ERROR;
                }
                if (log != null)
                {
                    log.LogInfo("ServoJ(multi-point) rtn: " + rtn);
                }
                return rtn;
            } else if (comType == 1) {
                ...
                for (int i = 0; i < joint_pos.size(); i++) {
                    JointPos jp = joint_pos.get(i);
                    if (i > 0) jointStr.append(",");
                    jointStr.append(String.format("{%.3f,%.3f,%.3f,%.3f,%.3f,%.3f}",
                            jp.J1, jp.J2, jp.J3, jp.J4, jp.J5, jp.J6));
                }
                jointStr.append("}");
                ...
                String cmdStr = String.format("ServoJ(%s,%s,%f,%f,%f,%f,%f,%d)",
                        jointStr.toString(), axisStr, acc, vel, cmdT, filterT, gain, id);
...
//            clientDownLoad.setTimeouts(5000, 15000, 5000);
    ...
    public int ExtDevGetUDPComParam(UDPComParam param)
    {
        ...
        try
        {
            ...
            if ((int)result[0] == 0)
            {
                ...
                param.selfConnect = (int)result[10];
     ...
     * @param pos 目标位置
     * @param ovl 速度百分比
     * @param blend 平滑参数(mm或ms)
     * @return 错误码
     */
    public int ExtAxisMove(ExaxisPos pos, double ovl, double blend)
    {
        if (IsSockComError())
        {
            return sockErr;
...
//            }
    ...
    public int ExtAxisSyncMoveJ(JointPos joint_pos, DescPose desc_pos, int tool, int user, double vel, double acc, double ovl, ExaxisPos epos, double blendT, int offset_flag, DescPose offset_pos)
    {
        ...
        try
        {
            ...
            Object[] desc =  { desc_pos.tran.x, desc_pos.tran.y, desc_pos.tran.z, desc_pos.rpy.rx, desc_pos.rpy.ry, desc_pos.rpy.rz };
            Object[] offect = { offset_pos.tran.x, offset_pos.tran.y, offset_pos.tran.z, offset_pos.rpy.rx, offset_pos.rpy.ry, offset_pos.rpy.rz };
            Object[] params = new Object[] {1, epos.axis1, epos.axis2, epos.axis3, epos.axis4, ovl, blendT};
            int rtn = (int)client.execute("ExtAxisMoveJ" , params);
            if (rtn != 0)
            {
                if (log != null)
    ...
    public int ExtAxisSyncMoveL(JointPos joint_pos, DescPose desc_pos, int tool, int user, double vel, double acc, double ovl, double blendR, ExaxisPos epos, int offset_flag, DescPose offset_pos)
    {
        ...
        try
        {
            int rtn = -1;
            Object[] offect = { offset_pos.tran.x, offset_pos.tran.y, offset_pos.tran.z, offset_pos.rpy.rx, offset_pos.rpy.ry, offset_pos.rpy.rz };
            Object[] params = new Object[] {1, epos.axis1, epos.axis2, epos.axis3, epos.axis4, ovl, blendR};
    ...
    public int ExtAxisSyncMoveC(JointPos joint_pos_p, DescPose desc_pos_p, int ptool, int puser, double pvel, double pacc, ExaxisPos epos_p, int poffset_flag, DescPose offset_pos_p, JointPos joint_pos_t, DescPose desc_pos_t, int ttool, int tuser, double tvel, double tacc, ExaxisPos epos_t, int toffset_flag, DescPose offset_pos_t, double ovl, double blendR)
    {
        ...
        try
        {
            ...
            Object[] params = new Object[] {1, epos_t.axis1, epos_t.axis2, epos_t.axis3, epos_t.axis4, ovl, blendR};
            rtn = (int)client.execute("ExtAxisMoveJ" , params);
            if (rtn != 0)
            {
                if (log != null)
...
//    }
    ...
    public int ServoJTStart(int comType)
    {
        ...
        try
        {
            if (comType == 0) {
                ...
                Frame frame = new Frame(cmdFrameCnt, 1199, cmdStr);
                int rtn = udpCmdClient.sendFrame(FrameHandle.packFrame(frame));
                if (rtn != 0) {
                    return RobotError.ERR_SOCKET_SEND_FAILED;
                }
     ...
     * @return  错误码
     */
     public int ServoJT(double[] torque, double interval, int comType)
    {
        if (IsSockComError())
        {
            return sockErr;
        ...
        int errcode = ServoJT(torque, interval, checkFlag, jPowerLimit, jVelLimit, comType);
        return errcode;
    }
    ...
    public int ServoJT(double[] torque, double interval, int checkFlag, double[] jPowerLimit, double[] jVelLimit, int comType)
    {
        ...
        try
        {
            if (comType == 0) {
                ...
                String torqueStr = String.format("{%.3f,%.3f,%.3f,%.3f,%.3f,%.3f}",
                        torque[0], torque[1], torque[2], torque[3], torque[4], torque[5]);
                
                String powerStr = String.format("{%.3f,%.3f,%.3f,%.3f,%.3f,%.3f}",
                        jPowerLimit[0], jPowerLimit[1], jPowerLimit[2],
                        jPowerLimit[3], jPowerLimit[4], jPowerLimit[5]);
                
                String velStr = String.format("{%.3f,%.3f,%.3f,%.3f,%.3f,%.3f}",
                        jVelLimit[0], jVelLimit[1], jVelLimit[2],
                        jVelLimit[3], jVelLimit[4], jVelLimit[5]);

                String cmdStr = String.format("ServoJT(%s,%f,%d,%s,%s)",
                        torqueStr, interval, checkFlag, powerStr, velStr);
//                System.out.printf("cmdStr is %s\n", cmdStr);
                Frame frame = new Frame(cmdFrameCnt, 1200, cmdStr);
                int rtn = udpCmdClient.sendFrame(FrameHandle.packFrame(frame));
//                System.out.printf("FrameHandle.packFrame(frame) is %s\n", FrameHandle.packFrame(frame));
                if (rtn < 0) {
                    return RobotError.ERR_SOCKET_SEND_FAILED;
                }
                cmdFrameCnt++;
    ...
    public int ServoJTEnd(int comType)
    {
        ...
        try
        {
            if (comType == 0) {
                ...
                Frame frame = new Frame(cmdFrameCnt, 1201, cmdStr);
                int rtn = udpCmdClient.sendFrame(FrameHandle.packFrame(frame));
                if (rtn != 0) {
                    return RobotError.ERR_SOCKET_SEND_FAILED;
                }
    ...
    public int SetWeldMachineCtrlMode(int mode, int ioType)
    {
        ...
        try{
            Object[] params = new Object[] {ioType, mode};
    ...
    public int SetFocusCalibPoint(int pointNum, DescPose point)
    {
        ...
        try {
            Object[] params = new Object[]{pointNum, point.tran.x, point.tran.y, point.tran.z, point.rpy.rx, point.rpy.ry, point.rpy.rz};
            int rtn = (int) client.execute("SetFocusCalibPoint", params);
            return rtn;
        } catch (Throwable e) {
            if (log != null) {
                log.LogError(Thread.currentThread().getStackTrace()[1].getMethodName(), Thread.currentThread().getStackTrace()[1].getLineNumber(),
    ...
    public int ComputeFocusCalib(int pointNum, DescTran resultPos, double[] accuracy) {
        ...
        try {
            Object[] params = new Object[]{pointNum};
            Object[] result = (Object[]) client.execute("ComputeFocusCalib", params);
            if ((int) result[0] == 0) {
                resultPos.x = (double) result[1];
                resultPos.y = (double) result[2];
                resultPos.z = (double) result[3];
                accuracy[0] = (double) result[4];
            }
            return (int) result[0];
        } catch (Throwable e) {
            if (log != null) {
    ...
    /**
     * @brief 开启焦点跟随
     * @param kp       比例参数，默认50.0
     * @param kpredict 前馈参数，默认19.0
     * @param aMax     最大角加速度限制，默认1440°/s^2
     * @param vMax     最大角速度限制，默认180°/s
     * @param type     锁定X轴指向(0-参考输入矢量；1-水平；2-垂直)
     * @return 错误码
     */
    public int FocusStart(double kp, double kpredict, double aMax, double vMax, int type) {
        ...
        try {
            Object[] params = new Object[]{kp, kpredict, aMax, vMax, type};
            int rtn = (int) client.execute("FocusStart", params);
    ...
    public int FocusEnd() {
        ...
        try {
            Object[] params = new Object[]{};
            int rtn = (int) client.execute("FocusEnd", params);
    ...
    public int SetFocusPosition(DescTran pos) {
        ...
        try {
            Object[] params = new Object[]{pos.x, pos.y, pos.z};
            int rtn = (int) client.execute("SetFocusPosition", params);
    ...
    public int SetEncoderUpgrade(String path) {
        ...
        try {
            Object[] params = new Object[]{};
            int rtn = (int) client.execute("SetEncoderUpgrade", params);
    ...
    /**
     * @brief 设置关节固件升级
     * @param type 升级文件类型；1-升级固件；2-升级从站配置文件
     * @param path 本地升级包全路径(D://zUP/XXXXX.bin)
     * @return 错误码
     */
    public int SetJointFirmwareUpgrade(int type, String path) {
        ...
        try {
            ...
            if (0 == errcode) {
                ...
                String pathInRobot = "/tmp/" + f.getName();
                for (int i = 1; i < 7; i++) {
    ...
    public int SetTorqueDetectionSwitch(int flag)
    {
        ...
        try {
            ...
            int rtn = (int)client.execute("SetTorqueDetectionSwitch", params);

            if (log != null)
            {
                log.LogInfo("SetTorqueDetectionSwitch:"+rtn);
            }
            return rtn;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int LaserTrackingLaserOnOff(int OnOff, int weldId)
    {
        ...
        try {
            Object[] params = new Object[]{OnOff,weldId};

            int rtn = (int)client.execute("LaserTrackingLaserOnOff", params);
    ...
    /**
     * @brief 激光跟踪开始结束函数
     * @param  OnOff 0-结束 1-开始
     * @param  coordId 激光外设工具坐标系编号
     * @return 错误码
     */
    public int LaserTrackingTrackOnOff(int OnOff, int coordId)
    {
        ...
        try {
            Object[] params = new Object[]{OnOff,coordId};

            int rtn = (int)client.execute("LaserTrackingTrackOnOff", params);
     ...
     * @param  vel 速度 单位%
     * @param  distance 最大寻位距离 单位mm
     * @param  timeout 寻位超时时间 单位ms
     * @param  posSensorNum 激光标定的工具坐标编号
     * @return 错误码
     */
    public int LaserTrackingSearchStart_xyz(int direction, int vel, int distance, int timeout, int posSensorNum)
    {
        ...
        try {
            Object[] params = new Object[]{direction,vel,distance,timeout,posSensorNum};

            int rtn = (int)client.execute("LaserTrackingSearchStart_xyz", params);
    ...
    /**
     * @brief 激光寻位-任意方向
     * @param  directionPoint 寻位输入的点的xyz左边
     * @param  vel 速度 单位%
     * @param  distance 最大寻位距离 单位mm
     * @param  timeout 寻位超时时间 单位ms
     * @param  posSensorNum 激光标定的工具坐标编号
    ...
    public int LaserTrackingSearchStart_point(DescTran directionPoint, int vel, int distance, int timeout, int posSensorNum)
    {
        ...
        try {
            Object[] params = new Object[]{6,vel,distance,timeout,posSensorNum,directionPoint.x,directionPoint.y,directionPoint.z};

            int rtn = (int)client.execute("LaserTrackingSearchStart_point", params);
    ...
    public int LaserTrackingSensorConfig(String ip, int port)
    {
        ...
        try {
            Object[] params = new Object[]{ip,port};

            int rtn = (int)client.execute("LaserTrackingSensorConfig", params);
    ...
    public int LaserTrackingSensorSamplePeriod(int period)
    {
        ...
        try {
            ...
            int rtn = (int)client.execute("LaserTrackingSensorSamplePeriod", params);
    ...
    /**
     * @brief 激光外设驱动加载
     * @param  type 激光外设驱动的协议类型 101-睿牛 102-创想 103-全视 104-同舟 105-奥太
     * @return 错误码
     */
    public int LoadPosSensorDriver(int type)
    {
        ...
        try {
            ...
            int rtn = (int)client.execute("LoadPosSensorDriver", params);
    ...
    public int UnLoadPosSensorDriver()
    {
        ...
        try {
            ...
            int rtn = (int)client.execute("UnLoadPosSensorDriver", params);
    ...
    /**
     * @brief 激光焊缝轨迹记录
     * @param  status 0-停止记录 1-实时跟踪  2-开始记录
    ...
    public int LaserSensorRecord1(int status, int delayTime)
    {
        ...
        try {
            Object[] params = new Object[]{status,delayTime};

            int rtn = (int)client.execute("LaserSensorRecord1", params);
    ...
    public int LaserSensorReplay(int delayTime, double speed)
    {
        ...
        try {
            Object[] params = new Object[]{3,delayTime,speed};

            int rtn = (int)client.execute("LaserSensorReplay", params);
    ...
    public int MoveLTR()
    {
        ...
        try {
            ...
            int rtn = (int)client.execute("MoveLTR", params);
    ...
    public int LaserSensorRecordandReplay(int delayMode, int delayTime, int delayDisExAxisNum, double delayDis, double sensitivePara, double speed)
    {
        ...
        try {
            Object[] params = new Object[]{4,delayMode,delayTime,delayDisExAxisNum,delayDis,sensitivePara,speed};

            int rtn = (int)client.execute("LaserSensorRecordandReplay", params);
    ...
    public int MoveToLaserRecordStart(int moveType, double ovl)
    {
        ...
        try {
            Object[] params = new Object[]{moveType,ovl};

            int rtn = (int)client.execute("MoveToLaserRecordStart", params);
    ...
    public int MoveToLaserRecordEnd(int moveType, double ovl)
    {
        ...
        try {
            ...
            int rtn = (int)client.execute("MoveToLaserRecordEnd", params);
    ...
    /**
     * @brief 运动到激光传感器寻位点
     * @param  moveFlag 运动类型：0-PTP；1-LIN
     * @param  ovl 速度缩放因子，0-100
     * @param  dataFlag 焊缝缓存数据选择：0-执行规划数据；1-执行记录数据
     * @param  plateType 板材类型：0-波纹板；1-瓦楞板；2-围栏板；3-油桶；4-波纹甲壳钢
     * @param  trackOffectType 激光传感器偏移类型：0-不偏移；1-基坐标系偏移；2-工具坐标系偏移；3-激光传感器原始数据偏移
     * @param  offset 偏移量
     * @return 错误码
     */
    public int MoveToLaserSeamPos(int moveFlag, double ovl, int dataFlag, int plateType, int trackOffectType, DescPose offset)
    {
        ...
        try {
            Object[] params = new Object[]{moveFlag,ovl,dataFlag,plateType,trackOffectType,offset.tran.x,offset.tran.y,offset.tran.z,offset.rpy.rx,offset.rpy.ry,offset.rpy.rz};
            Object[] param = new Object[]{params};
            int rtn = (int)client.execute("MoveToLaserSeamPos", param);

            if (log != null)
            {
                log.LogInfo("MoveToLaserSeamPos:"+rtn);
            }
            return rtn;
        } catch (Throwable e) {
            if (log != null) {
     ...
     * @param  offset 偏移量
     * @param  jPos 关节位置[°]
     * @param  descPos 笛卡尔位置[mm]
     * @param  tool 工具坐标系
     * @param  user 工件坐标系
     * @param  exaxis 扩展轴位置[mm]
     * @return 错误码
     */
    public int GetLaserSeamPos(int trackOffectType, DescPose offset, JointPos jPos, DescPose descPos, int[] tool, int[] user, ExaxisPos exaxis)
    {
        ...
        try {
            Object[] params = new Object[]{trackOffectType,offset.tran.x,offset.tran.y,offset.tran.z,offset.rpy.rx,offset.rpy.ry,offset.rpy.rz};
            Object[] param = new Object[]{params};

            Object[] result = (Object[])client.execute("GetLaserSeamPos", param) ;
            int rtn = (int)result[0];
            if(rtn==0){
                String paramStr = (String)result[1];

                // 假设 result 是一个数组或列表，result[1] 是字符串类型
                System.out.println(paramStr + "\n");

                // 使用split方法分割字符串
                String[] parS = paramStr.split(",");
                if (parS.length != 20) {
                    log.LogInfo("get SeamPos Point size fail");
                    return -1;
                }

                // 假设 jPos 是一个包含 jPos 数组的对象
                jPos.J1 = Double.parseDouble(parS[0]);
                jPos.J2 = Double.parseDouble(parS[1]);
                jPos.J3 = Double.parseDouble(parS[2]);
                jPos.J4 = Double.parseDouble(parS[3]);
                jPos.J5 = Double.parseDouble(parS[4]);
                jPos.J6 = Double.parseDouble(parS[5]);

                // 假设 descPos 是一个包含 tran 和 rpy 的对象
                descPos.tran.x = Double.parseDouble(parS[6]);
                descPos.tran.y = Double.parseDouble(parS[7]);
                descPos.tran.z = Double.parseDouble(parS[8]);
                descPos.rpy.rx = Double.parseDouble(parS[9]);
                descPos.rpy.ry = Double.parseDouble(parS[10]);
                descPos.rpy.rz = Double.parseDouble(parS[11]);

                // 解析整数
                tool[0] = Integer.parseInt(parS[12]);
                user[0] = Integer.parseInt(parS[13]);

                // 假设 exaxis 是一个包含 ePos 数组的对象
                exaxis.axis1 = Double.parseDouble(parS[16]);
                exaxis.axis2 = Double.parseDouble(parS[17]);
                exaxis.axis3 = Double.parseDouble(parS[18]);
                exaxis.axis4 = Double.parseDouble(parS[19]);
            }

            if (log != null)
            {
                log.LogInfo("GetLaserSeamPos:"+rtn);
            }
            return rtn;
        } catch (Throwable e) {
            if (log != null) {
     ...
     * @brief 阻抗启停控制
     * @param  status 0：关闭；1-开启
     * @param  workSpace 0-关节空间；1-迪卡尔空间
     * @param  forceThreshold 触发力阈值(N)
     * @param  m 质量参数
     * @param  b 阻尼参数
     * @param  k 刚度参数
     * @param  maxV 最大线速度(mm/s)
     * @param  maxVA 最大线加速度(mm/s2)
     * @param  maxW 最大角速度(°/s)
     * @param  maxWA 最大角加速度(°/s2)
     * @return 错误码
     */
    public int ImpedanceControlStartStop(int status, int workSpace, double[] forceThreshold, double[] m, double[] b, double[] k, double maxV, double maxVA, double maxW, double maxWA)
    {
        ...
        try {
            Object[] params = new Object[]{status,workSpace,forceThreshold[0],forceThreshold[1],forceThreshold[2],
                    forceThreshold[3],forceThreshold[4],forceThreshold[5],m[0],m[1],m[2],m[3],m[4],m[5],
                    b[0],b[1],b[2],b[3],b[4],b[5],k[0],k[1],k[2],k[3],k[0],k[5],maxV,maxVA,maxW,maxWA};
            Object[] param = new Object[]{params};

            int rtn = (int)client.execute("ImpedanceControlStartStop", param) ;

            if (log != null)
            {
                log.LogInfo("ImpedanceControlStartStop:"+rtn);
            }
            return rtn;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int GetToolCoordWithID(int id, DescPose coord, int[] type, int[] install, int[] toolID, int[] loadNo)
    {
        ...
        try {
            ...
            Object[] result = (Object[]) client.execute("GetToolCoordWithID", params) ;
            int rtn=(int)result[0];
            ...
            if (log != null)
            {
                log.LogInfo("GetToolCoordWithID:"+rtn);
            }
            return rtn;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int GetWObjCoordWithID(int id, DescPose coord, int[] refFrame)
    {
        ...
        try {
            ...
            Object[] result = (Object[]) client.execute("GetWObjCoordWithID", params) ;
            int rtn=(int)result[0];
            ...
            if (log != null)
            {
                log.LogInfo("GetWObjCoordWithID:"+rtn);
            }
            return rtn;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int GetExToolCoordWithID(int id, DescPose coord, DescPose tcoord)
    {
        ...
        try {
            ...
            Object[] result = (Object[]) client.execute("GetExToolCoordWithID", params) ;
            int rtn=(int)result[0];
            ...
            if (log != null)
            {
                log.LogInfo("GetExToolCoordWithID:"+rtn);
            }
            return rtn;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int GetExAxisCoordWithID(int id, DescPose coord, int[] axisCoordNum, int[] calibFlag)
    {
        ...
        try {
            ...
            Object[] result = (Object[]) client.execute("GetExAxisCoordWithID", params) ;
            int rtn=(int)result[0];
            ...
            if (log != null)
            {
                log.LogInfo("GetExAxisCoordWithID:"+rtn);
            }
            return rtn;
        } catch (Throwable e) {
            if (log != null) {
     ...
     * @return 错误码
     */
    public int GetTargetPayloadWithID(int id, double[] weight, DescTran cog)
    {
        if (IsSockComError()) {
            return sockErr;
        }
        ...
        try {
            ...
            Object[] result = (Object[]) client.execute("GetTargetPayloadWithID", params) ;
            int rtn=(int)result[0];

            if(rtn==0){
                weight[0] = (double)result[1];
            ...
            if (log != null)
            {
                log.LogInfo("GetTargetPayloadWithID:"+rtn);
            }
            return rtn;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int GetCurToolCoord(DescPose coord)
    {
        ...
        try {
            ...
            if (log != null)
            {
                log.LogInfo(robot_state_pkg.toolCoord[0]+","+
                        robot_state_pkg.toolCoord[1]+","+
                        robot_state_pkg.toolCoord[2]+","+
                        robot_state_pkg.toolCoord[3]+","+
                        robot_state_pkg.toolCoord[4]+","+
                        robot_state_pkg.toolCoord[5]+": "+sockErr);
            }
            return errcode;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int SetLoadCoord(DescTran coord)
    {
        ...
        try {
            Object[] params = new Object[]{coord.x,coord.y,coord.z,0};

            int rtn= (int) client.execute("SetLoadCoord", params) ;

            if (log != null)
            {
                log.LogInfo("SetLoadCoord:"+rtn);
            }
            return rtn;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int SetLoadCoord(int loadNum, DescTran coord)
    {
        ...
        try {
            Object[] params = new Object[]{coord.x,coord.y,coord.z,loadNum};
    ...
    /**
     * @brief 设置自定义摆动参数
     * @param  id 自定义摆动编号：0-2
     * @param  pointNum 摆动点位个数 0-10
     * @param  point 移动端点数据x,y,z
     * @param  stayTime 摆动停留时间ms
     * @param  frequency 摆动频率 Hz
     * @param  incStayType 等待模式：0-周期不包含等待时间；1-周期包含等待时间
     * @param  stationary 摆动位置等待：0-等待时间内继续运动；1-等待时间内位置静止
     * @return  错误码
     */
    public int CustomWeaveSetPara(int id, int pointNum, DescTran[] point, double[] stayTime, double frequency, int incStayType, int stationary)
    {
        ...
        try {
            ...
            for (int i = 0; i < 10; i++)
            {
                param[i * 3 + 2] = point[i].x;
                param[i * 3 + 3] = point[i].y;
                param[i * 3 + 4] = point[i].z;

                param[i + 32] = stayTime[i];
            }
            ...
            Object[] params = new Object[]{param};
            int rtn = (int)client.execute("CustomWeaveSetPara", params) ;

            if (log != null)
            {
                log.LogInfo("CustomWeaveSetPara:"+rtn);
            }
            return rtn;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int CustomWeaveGetPara(int id, int[] pointNum, DescTran[] point, double[] stayTime, double[] frequency, int[] incStayType, int[] stationary)
    {
        ...
        try {
            Object[] params = new Object[]{id};
            Object[] result = (Object[])client.execute("CustomWeaveGetPara", params) ;
            int rtn=(int)result[0];
            if(rtn==0) {
                String rtnStr = (String) result[1];
                String[] parS = rtnStr.split(",");
                if (parS.length != 44) {
                    log.LogInfo("CustomWeaveGetPara size fail");
                    return -1;
                }
                pointNum[0] = Integer.parseInt(parS[0]);
                for (int i = 0; i < 10; i++) {
                    point[i].x = Double.parseDouble(parS[i * 3 + 1]);
                    point[i].y = Double.parseDouble(parS[i * 3 + 2]);
                    point[i].z = Double.parseDouble(parS[i * 3 + 3]);
                    stayTime[i] = Double.parseDouble(parS[i + 31]);
                }
                frequency[0] = Double.parseDouble(parS[41]);
                incStayType[0] = Integer.parseInt(parS[42]);
                stationary[0] = Integer.parseInt(parS[43]);
            }
                if (log != null)
                {
                    log.LogInfo("CustomWeaveGetPara:"+rtn);
                }
                return rtn;
        } catch (Throwable e) {
                if (log != null) {
    ...
    public int KernelUpgrade(String filePath)
    {
        ...
        try {
            ...
            {
                Object[] params=new Object[]{};
                errcode = (int)client.execute("KernelUpgrade", params) ;

                if (log != null)
                {
                     log.LogInfo("KernelUpgrade:"+errcode);
                }
            }
            return errcode;
        } catch (Throwable e) {
    ...
    public int GetKernelUpgradeResult(int[] result)
    {
        ...
        try {
            Object[] params=new Object[]{};
            Object[] results = (Object[])client.execute("GetKernelUpgradeResult", params) ;
            int rtn=(int)results[0];

            if(rtn==0){
                result[0]=(int)results[1];
            }
            if (log != null)
            {
                log.LogInfo("GetKernelUpgradeResult:"+rtn);
            }
...
//    }
    ...
    public int MoveToTPDStart(String name, int moveType, double ovl)
    {
        ...
        try {
            Object[] params = new Object[]{name, moveType, ovl};
            errcode = (int) client.execute("MoveToTPDStart", params);

            if (log != null) {
                log.LogInfo("MoveToTPDStart: " + errcode);
            }
            return errcode;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int SetAxleGenComEnable(int mode) {
        ...
        try {
            Object[] params = new Object[]{mode};
            int errcode = (int) client.execute("SetAxleGenComEnable", params);

            if (log != null) {
                log.LogInfo("SetAxleGenComEnable: " + errcode);
            }
            return errcode;
        } catch (Throwable e) {
            if (log != null) {
     ...
     * @return 错误码
     */
    public int GetAxleGenComCycleData(int len, int[] cycleData) {
        if (IsSockComError()) {
            return sockErr;
        }
        ...
        try {
            ...
            Object[] result = (Object[]) client.execute("GetAxleGenComCycleData", params);
            int errcode = (int) result[0];
            ...
            if (errcode == 0) {
                ...
                if (log != null) {
                    log.LogError(Thread.currentThread().getStackTrace()[1].getMethodName(),
                            Thread.currentThread().getStackTrace()[1].getLineNumber(),
                            "execute GetAxleGenComCycleData fail: " + errcode);
                }
            }
            return errcode;
        } catch (Throwable e) {
    ...
    public int SndRcvAxleGenComCmdData(int lenSnd, int[] sndBuff, int lenRcv, int[] rcvData) {
        ...
        try {
            Object[] tmpSenbuf =new Object[lenSnd];
            for (int i = 0; i < lenSnd; i++)
            {
                tmpSenbuf[i] = sndBuff[i];
            }
            Object[] params = new Object[]{lenSnd, tmpSenbuf, lenRcv};
            Object[] result = (Object[]) client.execute("SndRcvAxleGenComCmdData", params);
            int errcode = (int) result[0];
            ...
            if (errcode == 0) {
                ...
                if (log != null) {
                    log.LogError(Thread.currentThread().getStackTrace()[1].getMethodName(),
                            Thread.currentThread().getStackTrace()[1].getLineNumber(),
                            "execute SndRcvAxleGenComCmdData fail: " + errcode);
                }
            }
            return errcode;
        } catch (Throwable e) {
     ...
     * @return 错误码
     */
    public int SetRobotStopOnComDisc(int pordID, boolean enable, int confirmTime) {
        if (IsSockComError()) {
            return sockErr;
        }

        try {
            Object[] tmpparams = new Object[]{pordID, enable ? 1 : 0, confirmTime};
            Object[] params = new Object[]{tmpparams};

            int errcode = (int) client.execute("SetRobotStopOnComDisc", params);

            if (log != null) {
                log.LogInfo("SetRobotStopOnComDisc: " + errcode);
            }
            return errcode;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int GetRobotStopOnComDisc(int pordID, int[] enable, int[] confirmTime) {
        ...
        try {
            Object[] tmp = new Object[]{pordID};
            Object[] params = new Object[]{tmp};

            Object[] result = (Object[]) client.execute("GetRobotStopOnComDisc", params);
            int errcode = (int) result[0];

            if (errcode == 0) {
                enable[0] = (int) result[1];
                confirmTime[0] = (int) result[2];
            } else {
                if (log != null) {
                    log.LogError(Thread.currentThread().getStackTrace()[1].getMethodName(),
                            Thread.currentThread().getStackTrace()[1].getLineNumber(),
                            "execute GetRobotStopOnComDisc fail: " + errcode);
                }
            }
            return errcode;
        } catch (Throwable e) {
...
/**
 * 验证帧字符串的格式是否正确
 * @param frameStr 待验证的帧字符串，格式如：/f/bIII20III303III7IIIMode(0)III/b/f
 * @return true 表示格式正确，false 表示格式错误
 */
public boolean VerifyFrame(String frameStr) {
    if (frameStr == null || frameStr.length() < 20) {
        return false;
    }
    // 检查帧头帧尾
    ...
    // 提取中间数据部分（去掉帧头帧尾）
    String data = frameStr.substring(4, frameStr.length() - 4);
    List<String> parts = new ArrayList<>();
    int start = 0;

    // 按 "III" 分割前5个字段（包括可能为空的第一个字段）
    for (int i = 0; i < 5; i++) {
        int pos = data.indexOf("III", start);
        if (pos == -1) {
            return false; // 缺少分隔符
        }
        parts.add(data.substring(start, pos));
        start = pos + 3;
    }
    // 添加剩余部分（最后一个分隔符之后的内容，可能为空）
    ...
    try {
        ...
        // 检查关键字段（索引1~4）是否非空
        if (parts.get(1).isEmpty() || parts.get(2).isEmpty() ||
            parts.get(3).isEmpty() || parts.get(4).isEmpty()) {
            return false;
        }
        // 验证内容长度字段（索引3）是否等于实际内容（索引4）的长度
        int contentLen = Integer.parseInt(parts.get(3));
        return contentLen == parts.get(4).length();
    } catch (NumberFormatException e) {
        return false; // 内容长度字段无法解析为整数
    }
}

/**
 * UDP 发送一帧指令
 * @param frame 待发送的帧字符串，格式如：/f/bIII20III303III7IIIMode(0)III/b/f
 * @return 错误码（0 表示成功，非零表示失败）
 */
public int SendUDPFrame(String frame) {
    // 1. 验证帧格式
    if (!VerifyFrame(frame)) {
        return RobotError.ERR_PARAM_VALUE;  // 参数错误
    }

    // 2. 发送数据
    int rtn = udpCmdClient.sendFrame(frame);
    if (rtn < 0) {
        return RobotError.ERR_SOCKET_SEND_FAILED;  // 发送失败
    }
...
//                    maxJointVel[2],maxJointVel[3],maxJointVel[4],maxJointVel[5]};
            ...
            if (log != null) {
                log.LogInfo("SetVelReducePara: " + errcode);
            }
            return errcode;
        } catch (Throwable e) {
            if (log != null) {
    ...
    /**
     * @brief 定点摆动开始
     * @param weaveNum 摆动编号[0-7]
     * @param mode 0-工具坐标系；1-参考点
     * @param refPoint 参考点笛卡尔坐标[x,y,z,a,b,c]
     * @param weaveTime 摆动时间[s]
     * @return 错误码
     */
    public int OriginPointWeaveStart(int weaveNum, int mode, DescPose refPoint, double weaveTime) {
        ...
        try {
            Object[] tmp = new Object[]{weaveNum, mode, 
                    refPoint.tran.x, refPoint.tran.y, refPoint.tran.z,
                    refPoint.rpy.rx, refPoint.rpy.ry, refPoint.rpy.rz, 
                    weaveTime};
                    
            Object[] params = new Object[]{tmp};
            int errcode = (int) client.execute("OriginPointWeaveStart", params);

            if (log != null) {
                log.LogInfo("OriginPointWeaveStart: " + errcode);
            }
            return errcode;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int OriginPointWeaveEnd() {
        ...
        try {
            ...
            Object[] params = new Object[]{tmp};
            int errcode = (int) client.execute("OriginPointWeaveEnd", params);

            if (log != null) {
                log.LogInfo("OriginPointWeaveEnd: " + errcode);
            }
            return errcode;
        } catch (Throwable e) {
            if (log != null) {
...
>>>>>>> 3.9.9
    ...
    public int SetDIConfig(int[] config) {
        ...
        try {
            ...
            Integer[] configArray = new Integer[8];
            for (int i = 0; i < 8; i++) {
                configArray[i] = config[i];
            }
            params[0] = configArray;
            int errcode = (int) client.execute("SetDIConfig", params);

            if (log != null) {
                log.LogInfo("SetDIConfig: " + errcode);
            }
            return errcode;
        } catch (Throwable e) {
            if (log != null) {
...
>>>>>>> 3.9.9
    ...
    public int GetDIConfig(int[] config) {
        ...
        try {
            Object[] params = new Object[]{};
            Object[] result = (Object[]) client.execute("GetDIConfig", params);
            int errcode = (int) result[0];
            ...
            if (errcode == 0) {
                ...
                if (log != null) {
                    log.LogError(Thread.currentThread().getStackTrace()[1].getMethodName(),
                            Thread.currentThread().getStackTrace()[1].getLineNumber(),
                            "execute GetDIConfig fail: " + errcode);
                }
            }
            return errcode;
        } catch (Throwable e) {
...
>>>>>>> 3.9.9
    ...
    public int SetDOConfig(int[] config) {
        ...
        try {
            ...
            params[0] = configArray;
            int errcode = (int) client.execute("SetDOConfig", params);

            if (log != null) {
                log.LogInfo("SetDOConfig: " + errcode);
            }
            return errcode;
        } catch (Throwable e) {
            if (log != null) {
...
>>>>>>> 3.9.9
    ...
    public int GetDOConfig(int[] config) {
        ...
        try {
            Object[] params = new Object[]{};
            Object[] result = (Object[]) client.execute("GetDOConfig", params);
            int errcode = (int) result[0];
            ...
            if (errcode == 0) {
                ...
                if (log != null) {
                    log.LogError(Thread.currentThread().getStackTrace()[1].getMethodName(),
                            Thread.currentThread().getStackTrace()[1].getLineNumber(),
                            "execute GetDOConfig fail: " + errcode);
                }
            }
            return errcode;
        } catch (Throwable e) {
    ...
    public int SetToolDIConfig(int[] config) {
        ...
        try {
            Object[] params = new Object[1];
            Integer[] configArray = new Integer[2];
            for (int i = 0; i < 2; i++) {
                configArray[i] = config[i];
            }
            params[0] = configArray;
            int errcode = (int) client.execute("SetToolDIConfig", params);

            if (log != null) {
                log.LogInfo("SetToolDIConfig: " + errcode);
            }
            return errcode;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int GetToolDIConfig(int[] config) {
        ...
        try {
            Object[] params = new Object[]{};
            Object[] result = (Object[]) client.execute("GetToolDIConfig", params);
            int errcode = (int) result[0];
            ...
            if (errcode == 0) {
                ...
                if (log != null) {
                    log.LogError(Thread.currentThread().getStackTrace()[1].getMethodName(),
                            Thread.currentThread().getStackTrace()[1].getLineNumber(),
                            "execute GetToolDIConfig fail: " + errcode);
                }
            }
            return errcode;
        } catch (Throwable e) {
    ...
    public int SetDIConfigLevel(int[] config) {
        ...
        try {
            ...
            params[0] = configArray;
            int errcode = (int) client.execute("SetDIConfigLevel", params);

            if (log != null) {
                log.LogInfo("SetDIConfigLevel: " + errcode);
            }
            return errcode;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int GetDIConfigLevel(int[] config) {
        ...
        try {
            Object[] params = new Object[]{};
            Object[] result = (Object[]) client.execute("GetDIConfigLevel", params);
            int errcode = (int) result[0];
            ...
            if (errcode == 0) {
                ...
                if (log != null) {
                    log.LogError(Thread.currentThread().getStackTrace()[1].getMethodName(),
                            Thread.currentThread().getStackTrace()[1].getLineNumber(),
                            "execute GetDIConfigLevel fail: " + errcode);
                }
            }
            return errcode;
        } catch (Throwable e) {
    ...
    public int SetDOConfigLevel(int[] config) {
        ...
        try {
            ...
            params[0] = configArray;
            int errcode = (int) client.execute("SetDOConfigLevel", params);

            if (log != null) {
                log.LogInfo("SetDOConfigLevel: " + errcode);
            }
            return errcode;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int GetDOConfigLevel(int[] config) {
        ...
        try {
            Object[] params = new Object[]{};
            Object[] result = (Object[]) client.execute("GetDOConfigLevel", params);
            int errcode = (int) result[0];
            ...
            if (errcode == 0) {
                ...
                if (log != null) {
                    log.LogError(Thread.currentThread().getStackTrace()[1].getMethodName(),
                            Thread.currentThread().getStackTrace()[1].getLineNumber(),
                            "execute GetDOConfigLevel fail: " + errcode);
                }
            }
            return errcode;
        } catch (Throwable e) {
    ...
    public int SetToolDIConfigLevel(int[] config) {
        ...
        try {
            ...
            params[0] = configArray;
            int errcode = (int) client.execute("SetToolDIConfigLevel", params);

            if (log != null) {
                log.LogInfo("SetToolDIConfigLevel: " + errcode);
            }
            return errcode;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int GetToolDIConfigLevel(int[] config) {
        ...
        try {
            Object[] params = new Object[]{};
            Object[] result = (Object[]) client.execute("GetToolDIConfigLevel", params);
            int errcode = (int) result[0];
            ...
            if (errcode == 0) {
                ...
                if (log != null) {
                    log.LogError(Thread.currentThread().getStackTrace()[1].getMethodName(),
                            Thread.currentThread().getStackTrace()[1].getLineNumber(),
                            "execute GetToolDIConfigLevel fail: " + errcode);
                }
            }
            return errcode;
        } catch (Throwable e) {
    ...
    public int SetStandardDILevel(int[] config) {
        ...
        try {
            ...
            params[0] = configArray;
            int errcode = (int) client.execute("SetStandardDILevel", params);

            if (log != null) {
                log.LogInfo("SetStandardDILevel: " + errcode);
            }
            return errcode;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int GetStandardDILevel(int[] config) {
        ...
        try {
            Object[] params = new Object[]{};
            Object[] result = (Object[]) client.execute("GetStandardDILevel", params);
            int errcode = (int) result[0];
            ...
            if (errcode == 0) {
                ...
                if (log != null) {
                    log.LogError(Thread.currentThread().getStackTrace()[1].getMethodName(),
                            Thread.currentThread().getStackTrace()[1].getLineNumber(),
                            "execute GetStandardDILevel fail: " + errcode);
                }
            }
            return errcode;
        } catch (Throwable e) {
    ...
    public int SetStandardDOLevel(int[] config) {
        ...
        try {
            ...
            params[0] = configArray;
            int errcode = (int) client.execute("SetStandardDOLevel", params);

            if (log != null) {
                log.LogInfo("SetStandardDOLevel: " + errcode);
            }
            return errcode;
        } catch (Throwable e) {
            if (log != null) {
    ...
    public int GetStandardDOLevel(int[] config) {
        ...
        try {
            Object[] params = new Object[]{};
            Object[] result = (Object[]) client.execute("GetStandardDOLevel", params);
            int errcode = (int) result[0];
            ...
            if (errcode == 0) {
                ...
                if (log != null) {
                    log.LogError(Thread.currentThread().getStackTrace()[1].getMethodName(),
                            Thread.currentThread().getStackTrace()[1].getLineNumber(),
                            "execute GetStandardDOLevel fail: " + errcode);
                }
            }
            return errcode;
        } catch (Throwable e) {
    ...
    public int SetExAxisCmdDoneTime(double time) {
        ...
        try {
            Object[] tmp = new Object[]{time};
            Object[] params = new Object[]{tmp};
            int errcode = (int) client.execute("SetExAxisCmdDoneTime", params);

            if (log != null) {
                log.LogInfo("SetExAxisCmdDoneTime: " + errcode);
            }
            return errcode;
        } catch (Throwable e) {
            if (log != null) {
     ...
     * @param r 末端红灯控制；false-灭；true-亮
     * @param g 末端绿灯控制；false-灭；true-亮
     * @param b 末端蓝灯控制；false-灭；true-亮
     * @return 错误码
     */
    public int SetUserLEDColor(boolean r, boolean g, boolean b) {
        if (IsSockComError()) {
            return sockErr;
        }

        try {
            Object[] params = new Object[]{r ? 1 : 0, g ? 1 : 0, b ? 1 : 0};
            int rtn = (int) client.execute("SetUserLEDColor", params);
            if (log != null) {
                log.LogInfo("SetUserLEDColor(" + (r ? 1 : 0) + ", " + (g ? 1 : 0) + ", " + (b ? 1 : 0) + ") : " + rtn);
            }
            return rtn;
        } catch (Throwable e) {
            if (log != null) {
    ...
    /**
     * @brief 删除开放协议Lua文件
     * @param fileName 要删除的开放协议lua文件名“CtrlDev_XXX.lua”
     * @return 错误码
     */
    public int OpenLuaDelete(String fileName) {
        ...
        int rtn = FileDelete(11, fileName);
        if (log != null) {
            log.LogInfo("OpenLuaDelete(" + fileName + ") : " + rtn);
        }
        return rtn;
    }
...
=======
     ...
     * @return  错误码
     */
    public int FT_SpiralSearch(int rcs, double dr, double ft, double max_t_ms, double max_vel, int strategy)
    {
        if (IsSockComError())
        {
            return sockErr;
        ...
        try
        {
            Object[] params = new Object[]{rcs, dr, ft, max_t_ms, max_vel, strategy};
            int rtn = (int) client.execute("FT_SpiralSearch", params);
...
=======
     ...
     * @param checksum 校验和 8位16进制
     * @return 错误码
     */
    public int GetSafetyParamsCheckSum(int[] status, long[] checksum)
    {
        if (IsSockComError())
        {
            return sockErr; // 使用sockErr替代g_sock_com_err
        }
        ...
        {
            Object[] params = new Object[]{};
            Object[] result = (Object[]) client.execute("GetSafetyParamsCheckSum", params);

            // 处理返回值 - 可能是Double类型
            errcode = ((Number) result[0]).intValue();
            if (errcode == 0)
            {
                status[0] = ((Number) result[1]).intValue();
                // 处理checksum，可能是Double或Long
                if (result[2] instanceof Number) {
                    checksum[0] = ((Number) result[2]).longValue() & 0xFFFFFFFFL;
                } else {
                    checksum[0] = 0;
                }
            }
            ...
            if (log != null)
            {
                log.LogInfo(String.format("GetSafetyParamsCheckSum(ref %d,ref 0x%08X) : %d",
                        status[0], checksum[0], errcode));
            }
            return errcode;
        }
        catch (Throwable e)
    ...
    public int SafetyOPPasswordCheck(int status, String password)
    {
        ...
        try
        {
            Object[] params = new Object[]{status, password};
            // 可能返回值不是Object数组，而是单个值
            Object result = client.execute("SafetyOPPasswordCheck", params);

            // 处理返回值 - 可能是Double或Integer
            if (result instanceof Number) {
                errcode = ((Number) result).intValue();
            } else if (result instanceof Object[]) {
                Object[] resultArray = (Object[]) result;
                errcode = ((Number) resultArray[0]).intValue();
            } else {
                errcode = RobotError.ERR_RPC_ERROR;
            }

            if (log != null)
            {
                log.LogInfo(String.format("SafetyOPPasswordCheck(%d,%s) : %d",
                        status, password, errcode));
            }
            return errcode;
        }
        catch (Throwable e)
    ...
    */
    public int SetRobotTime()
    {
        ...
        String cmdStr = "SetQNXSystemTime(\"" + dateStr + "\",\"" + timeStr + "\")";

        Frame frame = new Frame(cmdFrameCnt, 343, cmdStr);
        int rtn = udpCmdClient.sendFrame(FrameHandle.packFrame(frame));
        if (rtn <= 0) {
            return RobotError.ERR_SOCKET_SEND_FAILED;
        }
        ...
        if (log != null) {
            log.LogInfo("SetRobotTime() : " + cmdStr);
        }
    ...
    public int GripperWaitMotionDone(int staus, int timeout, int strategy, int type)
    {
        ...
        try
        {
            Object[] params = new Object[]{staus, timeout, strategy, type};
            int rtn = (int) client.execute("GripperWaitMotionDone", params);
            if (log != null)
            {
    ...
    */
    public int SetSafetyDIConfig(int ID, int config)
    {
        ...
        try
        {
            Object[] params = new Object[]{ID, config};
            int errcode = (int) client.execute("SetSafetyDIConfig", params);
            if (0 != errcode)
            {
                if (log != null) {
    ...
    * @return 错误码
    ...
    public int SetSafetyDOConfig(int ID, int config)
    {
        ...
        {
            Object[] params = new Object[]{ID, config};
            int errcode = (int) client.execute("SetSafetyDOConfig", params);
            if (0 != errcode)
            {
                if (log != null) {
    ...
    public int HiSpeedManualSwitch(int state)
    {
        ...
        {
            Object[] params = new Object[]{state};
            int errcode = (int) client.execute("HiSpeedManualSwitch", params);
            return errcode;
        }
        catch (Throwable e)
        {
